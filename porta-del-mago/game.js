/* ============================================================
   LA PORTA DEL MAGO — motore di gioco (vanilla JS)
   Stesso impianto di "I Mondi Spirituali" e "Officina Alkemica".
   ============================================================ */
(function () {
  "use strict";
  ...
  const LS = { lang: "sc-lang", theme: "sctheme", best: "sc-pm-best", seen: "sc-pm-seen" };
  ...
  const MODES = ["stato", "lettura", "risveglio", "quiz", "memoria"];
  const ROMAN = { stato: "I", lettura: "II", risveglio: "III", quiz: "IV", memoria: "V" };
  const GLYPH = { stato: "◐", lettura: "◇", risveglio: "☉", quiz: "✦", memoria: "✶" };
  ...
  function buildLettura() {
    return shuffle(D.LETTURA).map(s => ({
      kind: "lettura",
      media: () => frag(`<div class="q-desc">${esc(L(s.sit))}</div>`),
      prompt: () => t("letturaPrompt"),
  ...
  function buildRisveglio() {
    return shuffle(D.RISVEGLIO).map(p => ({
      kind: "risveglio",
      media: () => frag(`<div class="persona-item">${esc(L(p.item))}</div>`),
      prompt: () => t("risveglioPrompt"),
      options: [
        { it: I.STR.risveglio_opt0.it, en: I.STR.risveglio_opt0.en, correct: p.belongs === 0 },
        { it: I.STR.risveglio_opt1.it, en: I.STR.risveglio_opt1.en, correct: p.belongs === 1 }
      ],
      recap: { it: `<b>${p.item.it}</b> — ${p.note.it}`, en: `<b>${p.item.en}</b> — ${p.note.en}` }
    }));
  }
  ...
  function startMode(mode) {
    ...
    else if (mode === "lettura") items = buildLettura();
    else if (mode === "risveglio") items = buildRisveglio();
