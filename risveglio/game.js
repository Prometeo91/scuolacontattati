/* ============================================================
   RISVEGLIO — motore di gioco (vanilla JS)
   Stesso impianto di "Officina Alkemica" e "La Porta del Mago".
   ============================================================ */
(function () {
  "use strict";

  const D = window.RI_DATA;
  const I = window.RI_I18N;

  const LS = { lang: "sc-lang", theme: "sctheme", best: "sc-ri-best", seen: "sc-ri-seen" };
  ...
  const MODES = ["stato", "ostacoli", "leggi", "quiz", "memoria"];
  const ROMAN = { stato: "I", ostacoli: "II", leggi: "III", quiz: "IV", memoria: "V" };
  const GLYPH = { stato: "◐", ostacoli: "◇", leggi: "☉", quiz: "✦", memoria: "✶" };
  ...
  function renderHome() {
    ...
    halo.appendChild(buildRingsDiagram());
    const words = L(I.STR.gameTitle).split(" ");
    const lastW = words.pop();
    halo.appendChild(frag(`<h1>${words.length ? esc(words.join(" ")) + " " : ""}<span class="accent">${esc(lastW)}</span></h1>`));
  ...
  function buildStato() {
    return shuffle(D.STATO).map(p => ({
      kind: "stato",
      media: () => frag(`<div class="persona-item">${esc(L(p.item))}</div>`),
      prompt: () => t("statoPrompt"),
      options: [
        { it: I.STR.stato_opt0.it, en: I.STR.stato_opt0.en, correct: p.belongs === 0 },
        { it: I.STR.stato_opt1.it, en: I.STR.stato_opt1.en, correct: p.belongs === 1 }
      ],
      recap: { it: `<b>${p.item.it}</b> — ${p.note.it}`, en: `<b>${p.item.en}</b> — ${p.note.en}` }
    }));
  }

  function buildOstacoli() {
    return shuffle(D.OSTACOLI).map(s => ({
      kind: "ostacoli",
      media: () => frag(`<div class="q-desc">${esc(L(s.sit))}</div>`),
      prompt: () => t("ostacoliPrompt"),
      options: [
        { it: I.STR.ostacolo_opt0.it, en: I.STR.ostacolo_opt0.en, correct: s.correct === 0 },
        { it: I.STR.ostacolo_opt1.it, en: I.STR.ostacolo_opt1.en, correct: s.correct === 1 },
        { it: I.STR.ostacolo_opt2.it, en: I.STR.ostacolo_opt2.en, correct: s.correct === 2 }
      ],
  ...
  function buildLeggi() {
    return shuffle(D.LEGGI).map(s => ({
      kind: "leggi",
      media: () => frag(`<div class="q-desc">${esc(L(s.sit))}</div>`),
      prompt: () => t("leggiPrompt"),
      options: [
        { it: I.STR.legge_opt0.it, en: I.STR.legge_opt0.en, correct: s.correct === 0 },
        { it: I.STR.legge_opt1.it, en: I.STR.legge_opt1.en, correct: s.correct === 1 },
        { it: I.STR.legge_opt2.it, en: I.STR.legge_opt2.en, correct: s.correct === 2 },
        { it: I.STR.legge_opt3.it, en: I.STR.legge_opt3.en, correct: s.correct === 3 }
      ],
      recap: { it: s.note.it, en: s.note.en }
    }));
  }
  ...
  function startMode(mode) {
    ...
    let items;
    if (mode === "stato") items = buildStato();
    else if (mode === "ostacoli") items = buildOstacoli();
    else if (mode === "leggi") items = buildLeggi();
