/* ============================================================
   I MONDI SPIRITUALI — motore di gioco (vanilla JS)
   ============================================================ */
(function () {
  "use strict";

  const D = window.MS_DATA;
  const I = window.MS_I18N;
  ...
  const MODES = ["colori", "sottopiani", "abitanti", "quiz", "memoria"];
  const ROMAN = { colori: "I", sottopiani: "II", abitanti: "III", quiz: "IV", memoria: "V" };
  const GLYPH = { colori: "✶", sottopiani: "☉", abitanti: "◈", quiz: "✦", memoria: "❖" };

  let app;
  let currentScreen = renderHome; // funzione da ri-eseguire al cambio lingua
  ...
  function renderHome() {
    ...
    halo.appendChild(buildPlanesDiagram());
    ...
    halo.appendChild(frag(`<div class="home-sub">${esc(L(I.STR.gameKicker))} · ${esc(L(I.STR.fromAuthor))}</div>`));
    ...
    MODES.forEach(m => {
      ...
      let meta = "";
      if (m === "memoria") {
        meta = best[m] != null ? `${t("bestScore")}: ${best[m]} ${L(I.STR.moves).toLowerCase()}` : t("notYetPlayed");
      } else {
        meta = best[m] != null ? `${t("bestScore")}: ${best[m]} ${t("points")}` : t("notYetPlayed");
      }
      const card = frag(`
  ...
  function buildPlanesDiagram() {
    const wrap = frag(`<div class="planes-diagram" role="img" aria-label="${esc(L(I.STR.gameTitle))}"></div>`);
    const sizes = [132, 110, 90, 71, 53, 36, 20];
    sizes.forEach((s, i) => {
      const op = 0.16 + i * 0.12;
      const ring = document.createElement("div");
      ring.className = "ring";
      ring.style.width = s + "px";
      ring.style.height = s + "px";
      ring.style.borderColor = i === 0 ? "rgba(123,94,167,0.35)" : `rgba(201,151,58,${op.toFixed(2)})`;
  ...
  function buildColori() {
    const chosen = pickN(D.COLORS, 10);
    return chosen.map(c => {
      const distract = pickN(D.COLORS.filter(x => x !== c), 3);
      const opts = shuffle([
        { it: c.state.it, en: c.state.en, correct: true },
        ...distract.map(d => ({ it: d.state.it, en: d.state.en, correct: false }))
      ]);
      return {
        kind: "colori",
        media: () => {
          const w = frag(`<div class="swatch-wrap"></div>`);
          const s = document.createElement("div");
          s.className = "swatch";
          s.style.background = c.css;
          w.appendChild(s);
          w.appendChild(frag(`<div class="swatch-name">${esc(t("colorName"))} · <span class="cname">?</span></div>`));
          w.dataset.name = L(c.name);
          return w;
        },
        prompt: () => t("colorPrompt"),
        options: opts,
        recap: { it: `<b>${c.name.it}</b> → ${c.state.it}`, en: `<b>${c.name.en}</b> → ${c.state.en}` },
        onReveal: (card) => { const cn = card.querySelector(".cname"); if (cn) { cn.textContent = L(c.name); cn.parentElement.classList.add("revealed"); } }
      };
    });
  }

  function buildAbitanti() {
    const chosen = pickN(D.ENTITIES, 8);
    return chosen.map(e => {
      const distract = pickN(D.ENTITIES.filter(x => x !== e), 3);
      const opts = shuffle([
        { it: e.name.it, en: e.name.en, correct: true },
        ...distract.map(d => ({ it: d.name.it, en: d.name.en, correct: false }))
      ]);
      return {
        kind: "abitanti",
        media: () => frag(`
          <div>
            <div class="entity-group">${esc(L(e.group))}</div>
            <div class="q-desc">${esc(L(e.clue))}</div>
          </div>`),
        prompt: () => t("entityPrompt"),
        options: opts,
        recap: { it: `<b>${e.name.it}</b> — ${e.group.it}`, en: `<b>${e.name.en}</b> — ${e.group.en}` }
      };
    });
  }

  function buildQuiz() {
    const chosen = pickN(D.QUIZ, 10);
    return chosen.map(q => {
      const opts = shuffle(q.options.it.map((_, k) => ({
        it: q.options.it[k], en: q.options.en[k], correct: k === q.correct
      })));
      return {
        kind: "quiz",
        media: () => null,
  ...
  function buildSottopiani() {
    // viaggio in ordine: dal 7° (più denso) al 1° — risalita
    return D.SUBPLANES.map(sp => {
      const opts = shuffle(sp.options.it.map((_, k) => ({
        it: sp.options.it[k], en: sp.options.en[k], correct: k === sp.correct
      })));
      return {
        kind: "sottopiani",
        media: () => frag(`
          <div>
            <div class="entity-group">${esc(L(sp.title))} · ${sp.level}/7</div>
            <div class="q-desc">${esc(L(sp.desc))}</div>
          </div>`),
        prompt: () => L(sp.q),
        options: opts,
        recap: { it: `<b>${sp.title.it}</b> — ${sp.options.it[sp.correct]}`, en: `<b>${sp.title.en}</b> — ${sp.options.en[sp.correct]}` }
      };
    });
  }

  // ============================================================
  //  ROUND (quiz engine condiviso)
  // ============================================================
  let R = null;
  ...
  function startMode(mode) {
    ...
    let items;
    if (mode === "colori") items = buildColori();
    else if (mode === "abitanti") items = buildAbitanti();
    else if (mode === "quiz") items = buildQuiz();
    else if (mode === "sottopiani") items = buildSottopiani();
  ...
  function renderRound() {
    ...
    R.pipStates.forEach((st, i) => {
      let cls = "pip";
      if (st === "ok") cls += " done";
      else if (st === "no") cls += " wrong";
      else if (i === R.idx) cls += " current";
    ...
    const ctx = (R.mode === "sottopiani") ? `<div class="q-context">${esc(t("question"))} ${R.idx + 1} ${esc(t("of"))} ${total} · ${esc(t("ascendNote"))}</div>`
      : `<div class="q-context">${esc(t("question"))} ${R.idx + 1} ${esc(t("of"))} ${total}</div>`;
    card.appendChild(frag(ctx));
    ...
    const foot = frag(`<div class="round-foot"></div>`);
    card.appendChild(foot);
    node.appendChild(card);

    mount(node);

    // se già risposto (ri-render per cambio lingua), ripristina stato
    if (R.answered) restoreAnswered(card, optsWrap, foot, it);
  }

  function answer(i, card, optsWrap, node, it) {
    if (R.answered) return;
    R.answered = true;
    const chosen = it.options[i];
    const correct = !!chosen.correct;
    const btns = optsWrap.querySelectorAll(".option");
    btns.forEach((b, k) => {
      b.disabled = true;
    ...
    if (correct) {
      ...
      const bonus = Math.min(R.streak - 1, 5) * 20;
      R.score += 100 + bonus;
      R.correctCount += 1;
      R.pipStates[R.idx] = "ok";
    } else {
      R.streak = 0;
      R.pipStates[R.idx] = "no";
      R.missed.push(it);
    }
    if (it.onReveal) it.onReveal(card);

    // aggiorna chip
    const sc = node.querySelector(".round-stats");
    if (sc) {
      sc.innerHTML = `<span class="stat-chip">${esc(t("score"))} <b>${R.score}</b></span>
        <span class="stat-chip streak${R.streak >= 3 ? " hot" : ""}">${esc(t("streak"))} <b>${R.streak}</b></span>`;
    }
    // aggiorna pip corrente
  ...
  function restoreAnswered(card, optsWrap, foot, it) {
    // determina la scelta: ricalcolo non memorizzato → mostro solo corrette e feedback
    const btns = optsWrap.querySelectorAll(".option");
    btns.forEach((b, k) => {
      b.disabled = true;
      if (it.options[k].correct) b.classList.add("correct");
    ...
    if (it.onReveal) it.onReveal(card);
    const wasCorrect = R.pipStates[R.idx] === "ok";
    showFeedback(card, wasCorrect, it);
  }
  ...
  function finishRound() {
    ...
    renderResult({
      mode: R.mode, score: R.score, correct: R.correctCount, total: R.items.length,
      isRecord, missed: R.missed.slice()
    });
  }
  ...
  function renderResult(res) {
    ...
    if (res.mode === "memoria") {
      node.appendChild(frag(`<div class="result-score">${res.moves}<small>${esc(t("moves"))}</small></div>`));
    } else {
    ...
    // recap (voci sbagliate, o highlight)
  ...
  function startMemory() {
    const pairs = D.MEMORY; // 6 coppie → 12 tessere
    let cards = [];
    pairs.forEach((p, i) => {
      cards.push({ pid: i, label: p.a });
      cards.push({ pid: i, label: p.b });
    });
    ...
    if (M.flipped.length === 2) {
      ...
      if (a.pid === b.pid) {
        setTimeout(() => {
          ...
          M.flipped = [];
          M.lock = false;
          if (M.matchedCount === D.MEMORY.length) finishMemory();
          else renderMemory();
        }, 460);
      } else {
        setTimeout(() => {
  ...
  function finishMemory() {
    ...
    renderResult({ mode: "memoria", moves: M.moves, correct: D.MEMORY.length, total: D.MEMORY.length, isRecord: isRecord, missed: [] });
  }

  // ============================================================
  //  CHROME: back / lingua / tema
  // ============================================================
  function backButton() {
