...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if(themeBtn){
    ...
    function updThemeBtn(){
      ...
      themeBtn.innerHTML=isDark
        ?'<span class="theme-toggle-icon">'+soleSvg+'</span> Tema chiaro'
        :'<span class="theme-toggle-icon">'+lunaSvg+'</span> Tema scuro';
    }
    updThemeBtn();
    themeBtn.addEventListener('click',function(){
      ...
      if(nl && nl.classList.contains('open')){
        ...
        nt.innerHTML='\u2630';
        nt.setAttribute('aria-label','Apri menu');
      }
    });
  }
  ...
  function activateVideoFacade(facade) {
    ...
    iframe.title = 'Video di presentazione della Scuola ContattaTi';
  ...
  if(list1){
    ...
    L1.forEach(function(l){
      ...
      var bc,bt;
      if(st==='in-corso'){bc='badge badge-green';bt='&#9679; In corso';}
      else if(st==='passata'){bc='badge badge-muted';bt='Conclusa';}
      else if(isPr){bc='badge badge-gold';bt='Iscriviti';}
      else{bc='badge badge-purple';bt=mesiL[l.month-1]+' '+l.year;}
      var sfx=st==='in-corso'?' — In corso ora':isPr?' \u2736 Prossima':'';
      var w=document.createElement('div');w.className='lesson-wrap';
      w.innerHTML='<div class="event-row has-detail" role="button" tabindex="0" aria-expanded="false"><div class="event-date"><p class="event-day ev-num">'+toRoman(l.num)+'</p><p class="event-month">Anno 1</p></div><div class="event-vline"></div><div class="event-info"><p class="event-title">Lezione '+toRoman(l.num)+sfx+'</p><p class="event-sub">'+l.titolo+'</p><p style="font-size:13px;color:var(--text-muted);margin-top:2px;">'+l.day+' '+mesiL[l.month-1]+' '+l.year+'</p></div><span class="'+bc+'">'+bt+'</span><span class="detail-pill">Temi \u25be</span><span class="expand-arrow">\u25bc</span></div>'+panelHTML(l);
      list1.appendChild(w);
  ...
  function renderAnno(lista,listId,label){
    ...
    lista.forEach(function(l){
      ...
      if(!l.titolo){
        var dataBadge=hasDate?'<span class="'+bc+'">'+bt+'</span>':'<span class="badge badge-muted">In arrivo</span>';
        w.innerHTML='<div class="event-row" style="opacity:0.65;"><div class="event-date"><p class="event-day ev-num">'+toRoman(l.num)+'</p><p class="event-month">'+label+'</p></div><div class="event-vline"></div><div class="event-info"><p class="event-title">Lezione '+toRoman(l.num)+'</p>'+(hasDate?'<p class="event-sub">'+l.day+' '+mesiL[l.month-1]+' '+l.year+'</p>':'<p class="event-sub">Locandina in arrivo</p>')+'</div>'+dataBadge+'</div>';
        el.appendChild(w);
        return;
      }
      var badgeHTML=hasDate?'<span class="'+bc+'">'+bt+'</span>':'';
      var sfx=st==='in-corso'?' — In corso ora':isPr?' \u2736 Prossima':'';
      w.innerHTML='<div class="event-row has-detail" role="button" tabindex="0" aria-expanded="false"><div class="event-date"><p class="event-day ev-num">'+toRoman(l.num)+'</p><p class="event-month">'+label+'</p></div><div class="event-vline"></div><div class="event-info"><p class="event-title">Lezione '+toRoman(l.num)+sfx+'</p><p class="event-sub">'+l.titolo+'</p>'+(hasDate?'<p style="font-size:13px;color:var(--text-muted);margin-top:2px;">'+l.day+' '+mesiL[l.month-1]+' '+l.year+'</p>':'')+'</div>'+badgeHTML+'<span class="detail-pill">Temi \u25be</span><span class="expand-arrow">\u25bc</span></div>'+panelHTML(l);
      el.appendChild(w);
      var row=w.querySelector('.event-row'),panel=w.querySelector('.lesson-panel');
      row.addEventListener('click',function(){toggle(row,panel);});
      row.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle(row,panel);}});
  ...
  /* ANNO 5 */
  renderAnno(L5,'calendar-list-anno5','Anno 5');
  ...
  /* ANNO 7 */
  renderAnno(L7,'calendar-list-anno7','Anno 7');

  renderAnno(L6,'calendar-list-anno6','Anno 6');

  renderAnno(L4,'calendar-list-anno4','Anno 4');
  renderAnno(L2,'calendar-list-anno2','Anno 2');
  renderAnno(L3,'calendar-list-anno3','Anno 3');
  ...
  if(track){
    ...
    slides.forEach(function(s,i){
      ...
      s.setAttribute('aria-label','Citazione '+(i+1)+' di '+slides.length);
    });
  ...
  if(cForm){
    ...
    cForm.addEventListener('submit',function(ev){
      ...
      if(Date.now()-cStart<3000){
        showFeedback('error','Attendi un momento','Il modulo è stato inviato troppo velocemente. Riprova tra qualche secondo.');
        return;
      }
      ...
      if(hp&&hp.value){
        ...
        showFeedback('success','Messaggio inviato','Grazie, ti risponderemo presto.');
        cForm.reset();
        return;
      }
      ...
      fetch(cForm.action,{
        ...
        cBtn.disabled=false;
        if(resp.ok){
          showFeedback('success','✓ Messaggio inviato','Grazie! Abbiamo ricevuto il tuo messaggio e ti risponderemo il prima possibile.');
          cForm.reset();
          ...
          resp.json().then(function(json){
            var msg=(json&&json.errors&&json.errors.length)
              ...
              : 'Qualcosa è andato storto. Prova a scriverci direttamente via WhatsApp o email.';
            showFeedback('error','Errore nell\u2019invio',msg);
          }).catch(function(){
            showFeedback('error','Errore nell\u2019invio','Qualcosa è andato storto. Prova a scriverci direttamente via WhatsApp o email.');
          });
        }
      }).catch(function(){
        cBtn.classList.remove('is-loading');
        cBtn.disabled=false;
        showFeedback('error','Errore di connessione','Impossibile inviare il messaggio. Verifica la tua connessione e riprova, oppure scrivici direttamente via WhatsApp.');
      });
    });
  }
...
(function(){
  ...
  if(tog&&grid){
    ...
    tog.addEventListener('click',function(){
      ...
      tog.textContent=expanded?'Mostra meno \u25b4':'Mostra tutte le foto \u25be';
    });
  }
  ...
  function show(i){
    ...
    lbImg.src=srcs[cur];
    lbImg.alt='Foto '+(cur+1)+' di '+srcs.length;
